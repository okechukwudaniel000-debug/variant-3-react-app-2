...
class TextScramble {
  ...
  update() {
    ...
    // Write via textContent (not innerHTML): no per-frame HTML parsing and no
    // <span> node churn — that DOM thrash is what stuttered on mobile. The old
    // `.d-char` wrapper had no CSS, so nothing visual is lost. Skip the DOM
    // write entirely on frames where the string is unchanged.
