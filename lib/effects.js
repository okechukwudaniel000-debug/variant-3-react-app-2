...
export function scrambleObserve(root) {
  ...
  (root || document)
    .querySelectorAll('.sec-title, .h1-main, .h1-accent')
    .forEach((el) => {
...
function animNum(el) {
  ...
  (function tick(now) {
    ...
    el.textContent =
      (dec ? (to * ease).toFixed(1) : Math.floor(to * ease).toLocaleString()) + sfx;
  ...
  const onScroll = () => {
    if (topnav)
      topnav.style.boxShadow =
        window.scrollY > 40 ? '0 4px 28px rgba(0,0,0,.35)' : 'none';
  };
